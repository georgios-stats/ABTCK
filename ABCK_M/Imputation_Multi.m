function [Kriging_W, Var_krig]=Imputation_Multi(Zt,beta,rho,sigma2,AA,site,site_e,S,tau2)
...
%          cc=(c+I*m_error);
         ...
         Var_Zstt=Gauss_Cov2(sigma2{S}, AA{S}, site_e, site_e,tau2{S});
         
         Var_Zstt1=Gauss_Cov2(sigma2{S+1}, AA{S+1}, site_e, site_e,tau2{S+1});%Var_Zs(sigma2,AA,rho,site_e,S+1,tau2);
      
         invVar_krig1A=diag((1./diag((Var_Zstt1)))); %inv(Var_Zstt1);% %-lambda1*c1 % %
 
         %COV_MAA=Gauss_Cov2(1, AA{S}, site{S}, site{S},tau2{S}); %V_Multi(sigma2,AA,rho,site,S+1,tau2); 
         COV_MAA=Gauss_Cov2(sigma2{S}, AA{S}, site{S}, site{S},tau2{S});
         cAA=Gauss_Cov2(sigma2{S}, AA{S}, site_e, site{S},tau2{S})';%t_star(sigma2,AA,rho,site_e,site,S+1,tau2);   
         lambdaAA=(COV_MAA\cAA)'; 
         
         invVar_krigA = diag((1./diag((Var_Zstt-lambdaAA*cAA))));%inv(Var_Zstt-lambdaAA*cAA);%
         Var_krig= diag(invVar_krigA);
        % weight=diag(1./diag(invVar_krigA+ (rho{S+1}.*eye(size(site_e,1)))*(invVar_krig1A)*(rho{S+1}.*eye(size(site_e,1)))));%diag(1/diag(invVar_krigA/sigma2{S}+ (rho{S+1}^2*eye(size(site{S+1},1))*(invVar_krig1A))/sigma2{S+1}));
weight=diag(1./diag(invVar_krigA+ (rho{S+1}^2.*(invVar_krig1A))));
         %weight
        ...
        %(weight*(rho{S+1}.*invVar_krig1A)*(Zt{S+1}- mu_SPlus1))'
        
         Kriging_W= (weight*(invVar_krigA))*(mu1+lambda*(ZT{S}-mu))+weight*(rho{S+1}.*invVar_krig1A)*(Zt{S+1}- mu_SPlus1);
         %Kriging_W=  mvnrnd((weight*(invVar_krigA))*(mu1+lambda*(ZT{S}-mu))+weight*(rho{S+1}.*invVar_krig1A)*(Zt{S+1}- mu_SPlus1),inv(invVar_krigA+ (rho{S+1}^2.*(invVar_krig1A))),1);
