function [Zt_real,site_real,prCurr,AA,tau2, a_new,lim_all, KrigS,Var_krigS,Krig_site, Krig_W_etaMAT, Krig_site_eta]=...
    ABCK(PR_CURR,SITE_real,ZT_real,AA_E,LIM_ALL,a_new,a_NODES,NODES,S,low_l,site_e,n_e,BB,Delta,tau1,TAU2,bound)
...
if (S==2)
SITE{1}{1}=[SITE_real{1}{1}; SITE_real{1}{2}];%union(SITE_real{1}{1}, SITE_real{1}{2});%
n2N=size(SITE_real{1}{2},1);%(size(SITE{1}{1},1)-size(SITE_real{1}{1},1));
ZT{1}{1}=[ZT_real{1}{1}; ZT_real{1}{2}(1:n2N)+norm_r(0,0.02,size(n2N,1))];
SITE{1}{2}=[SITE_real{1}{2}];
ZT{1}{2}=[ZT_real{1}{2}];
end
if (S==1)
SITE{1}{1}=[SITE_real{1}{1}];%union(SITE_real{1}{1}, SITE_real{1}{2});%
ZT{1}{1}=[ZT_real{1}{1}];
end
    ...
    for ii=1:a_NODES
        if (NODES{ii}.inter==0)
           ...
           for jj=1:1 % to inprouve convergency we may use 3 or 4;     
            ...
            [New_Z{hh2}, Var_Z{hh2}]=Krig_MC(Zt_real{hh2},Beta_X{hh2},sigma2{hh2},AA{hh2},site_real{hh2},site_add{hh2},tt,tau2{hh2});
...
[Kriging_W,LB_W, UB_W, site_W]=KM_Augmented_Tree(Zt,Beta_X,sigma2,AA,site,site_e_t,a_new,S,tau2);
