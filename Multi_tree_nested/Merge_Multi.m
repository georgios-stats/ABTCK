function [site,Zt, n, prCurr1,AA,tau2, a, lim_ALL,NODES,a_NODES]=...
    Merge_Multi(prCurr,site,Zt,n,AA,S,lim_ALL,a,a_NODES,NODES, aalpha, bbeta,tau1,tau2,funname,funname2)
...
for  ii=1:a_NODES
    if (NODES{ii}.inter == 1)
        ...
        if (NODES{CH_L}.inter==0 && NODES{CH_R}.inter==0) % if both of the childerens are exterior.
            bb=bb+1; 
            PP(bb)=ii;  
            depth_each(bb)=floor(log(NODES{ii}.number)/log(2));% or better asta(bb)=ii;
        end
    end
end
