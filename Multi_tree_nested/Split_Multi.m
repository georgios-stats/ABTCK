function [site,Zt, n, prCurr1, AA,tau2,lim_ALL, a,Nodes,a_Nodes]= ...
    Split_Multi(prCurr,site,Zt,n,AA,S,low_l,lim_ALL,a,a_NODES,NODES, pprop, aalpha, bbeta,tau1,tau2,funname,funname2)
...
if (Min_n>low_l)% (Nnn1{1}>low_limit1 && Nnn1{2}>low_limit1 && Nnn2{1}>low_limit2 && Nnn2{2}>low_limit2)
    ...
    Zt_new1{(a_NODES+1)}=ZZt_new1{1}; 
    Zt_new1{(a_NODES+2)}=ZZt_new1{2};  
    %for t=1:S
...
%Proposal_AA
    ...
    % MH
    MH_ratio =(prod_prProp1+prod_prProp2-prod_prCurr-(prod_Proposal_AA))+(Prior_prop-Prior_curr)+(log(G)-log(P)); % ftia3e edw to RJ of split.
    u = log(rand);%
    if (u<MH_ratio)
    % accept
        % Updated tree
        Nodes = NODES_new; lim_ALL=lim_ALL_new; 
        a_Nodes=a_NODES_new; 
