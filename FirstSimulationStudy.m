...
% SITE_all1=lhsdesign((n+m),Dim_x)';
% SITE_all=[diag(lim_all(:,1))*ones(Dim_x, (n+m))+diag((lim_all(:,2)-lim_all(:,1)))*SITE_all1]';
% z=ones((m+n),1);
...
% [ipick,xsam,xobs,oL,isam]=cLHS(m,SITE_all,xobj,vobj,niter);
% LH_S1=SITE_all(ipick,:);
% SITE1=LH_S1;
%  x=SITE_all;
%  x(ipick, :) = [];
